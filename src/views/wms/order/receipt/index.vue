        ...
        <el-table-column label="商品明细" align="left" min-width="420" class-name="order-detail-cell">
          <template #default="scope">
            <order-detail-list
              ...
              variant="receipt"
              :tea-type-options="wms_tea_type"
              :harvest-season-options="wms_harvest_season"
            />
          </template>
        </el-table-column>
...
function getList() {
  ...
  listReceiptOrder(query).then(response => {
    ...
    detailLoading.value = Array(receiptOrderList.value.length).fill(false);
    loadAllReceiptOrderDetail();
    loading.value = false;
  });
}
...
/** 新增按钮操作 */
function handleAdd() {
  proxy.$router.push({ path: "/teaReceipt/receiptOrderEdit" });
}
...
function handleUpdate(row) {
  proxy.$router.push({ path: "/teaReceipt/receiptOrderEdit",  query: { id: row.id } });
}
...
function loadAllReceiptOrderDetail() {
  receiptOrderList.value.forEach(row => loadReceiptOrderDetail(row))
}
getList();
</script>
