...
function getList() {
  ...
  listMovementOrder(query).then(response => {
    ...
    detailLoading.value = Array(movementOrderList.value.length).fill(false);
    loadAllMovementOrderDetail();
    loading.value = false;
  });
}
...
/** 新增按钮操作 */
function handleAdd() {
  proxy.$router.push({ path: "/teaMovement/movementOrderEdit" });
}
...
function handleUpdate(row) {
  proxy.$router.push({ path: "/teaMovement/movementOrderEdit",  query: { id: row.id } });
}
...
function loadAllMovementOrderDetail() {
  movementOrderList.value.forEach(row => loadMovementOrderDetail(row))
}
getList();
</script>
<style lang="scss">
